...
int builtin_env(data_of_program *data)
{
	...
	/* prints if not arguments */
	if (data->tokens[1] == NULL)
		print_environ(data);
	else
	{
		for (i = 0; data->tokens[1][i]; i++)
		{/* this checks if exists a char = */
			if (data->tokens[1][i] == '=')
			...
			/* only temporally */
				...
				{/* this returns the old value of the var*/
...
/**
 * builtin_set_env - this prints
 * @data: the struct for the program's data
 * Return: prints zero if sucess, or other number
